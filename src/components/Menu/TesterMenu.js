...
const TesterMenu = ({ rolePermissions }) => {
  const theme = useTheme();
  const [open, setOpen] = React.useState(true);
  const [cardsPermission, setCardsPermission] = useState([]);

  useEffect(() => {
    const cardStates = localStorage.getItem('cardStates');
    const parsedCardStates = JSON.parse(cardStates);
